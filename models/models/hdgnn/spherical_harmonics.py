...
class SphericalHarmonicsHelper:
    ...
    def InitWignerDMatrix(self, edge_rot_mat):
        ...
        wigner_inv_y = torch.tensor([], device=self.device)
        ...
        for y_rot in self.y_rotations:
            ...
            wigner_inv_y = torch.cat([wigner_inv_y, (
                    torch.transpose(wignerD_reduce, 1, 2).contiguous().detach().unsqueeze(1)
                )], dim=1)
        ...
        wigner_inv_y = wigner_inv_y.view(-1, self.sphere_basis, self.sphere_basis_reduce)

        # wigner_inv_y = wigner_inv_y.transpose(2, 3).detach()
        # wigner_inv_y[:, [0:2], :, :] += wigner

        self.wigner = wigner.detach()
        self.wigner_inv = wigner_inv.detach()
        self.wigner_inv_y = wigner_inv_y.detach()
    ...
    def Rotate_edgeInv(self, edge_index, x):
        x_rot = torch.bmm(self.wigner[edge_index], x)
        return x_rot

    def Rotate_edge(self, edge_index, x):
        x_rot = torch.bmm(self.wigner[edge_index], x)
        return x_rot
...
class SphericalHarmonicsHelper_4:
    ...
    def Rotate(self, x):
        ...
        # print('{} {}'.format(self.wigner.size(), x.size()))
        x_rot = torch.bmm(self.wigner[:, :, :l], x)
